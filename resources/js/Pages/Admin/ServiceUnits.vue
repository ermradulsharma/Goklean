<template>
    <app-layout title="Service Units">
        <template #header>
            <h4 class="page-heading">Service Units</h4>
        </template>
        <template #actions>
            <Button @click="ExportExcel('xlsx')" type="button" class="export-btn p-button-success mx-5"
                :label="'Export'" />
        ...
        <div class="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <vue-good-table mode="remote" v-on:page-change="onPageChange" v-on:column-filter="onColumnFilter"
                ...
                :totalRows="serviceUnits.meta.pagination.total" :rows="serviceUnits.data" :rtl="$page.props.rtl">
                <template #table-row="props">

                    <!-- Status Column -->
                    <div v-if="props.column.field === 'status'">
                        <span :class="[props.row.status ? 'badge-success' : 'badge-danger', 'badge']">{{
                            props.row.status ? 'Active' : 'In-active' }}</span>
                    </div>
                    ...
                    <div v-else-if="props.column.field === 'actions'">
                        ...
                        <Button icon="pi pi-trash" class="p-button-rounded p-button-danger p-button-text" @click="deleteServiceUnit(props.row.id)" />
                    </div>
            ...
            <Sidebar position="right" v-model:visible="createForm" class="p-sidebar-md">
                <ServiceUnitForm :form-errors="errors" :cities="cities" @close="createForm = false;"
                    :title="'New ServiceUnit'" />
            </Sidebar>
            <Sidebar position="right" v-model:visible="editForm" class="p-sidebar-md">
                <ServiceUnitForm :form-errors="errors" :cities="cities" :edit-flag="editForm" @close="editForm = false;"
                    :service-unit-id="currentId" :title="'Edit Service Unit'" />
            </Sidebar>
        </div>
    </app-layout>
</template>
...
import ServiceUnitForm from '@/Pages/Admin/Forms/ServiceUnitForm';
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            options: {
                enabled: true,
                mode: 'pages',
                perPage: this.serviceUnits.meta.pagination.per_page,
                setCurrentPage: this.serviceUnits.meta.pagination.current_page,
    ...
    methods: {
        ...
        async ExportExcel() {
            ...
            const exportData = this.serviceUnits.data.map(row => {
                const rowData = {};
            ...
            link.setAttribute('download', 'surviceUnit');
        ...
        async fetchAllData() {
            const totalRows = this.serviceUnits.meta.pagination.total;
        ...
        //        this.$inertia.get('http://laravel.test/admin/export-survice-unit', this.getQueryParams(), {
        //           replace: false,
        //           preserveState: true,
        //           preserveScroll: true,
        //       });
        ...
        deleteServiceUnit(id) {
            this.$confirm.require({
                ...
                acceptLabel: 'Delete',
                accept: () => {
                    this.$inertia.delete(route('service-units.destroy', { id: id }), {}, {
                        onSuccess: () => {
                            this.$toast.add({
                                severity: 'info',
                                summary: 'Confirmed',
