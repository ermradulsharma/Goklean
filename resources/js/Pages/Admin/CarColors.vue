<template>
    <app-layout title="Car Colors">
        <template #header>
            <h4 class="page-heading">Car Colors</h4>
        </template>
        <template #actions>
        ...
        <div class="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <vue-good-table mode="remote" v-on:page-change="onPageChange" v-on:column-filter="onColumnFilter" v-on:per-page-change="onPerPageChange"
                            ...
                            :totalRows="carColors.meta.pagination.total"
                            :rows="carColors.data" :rtl="$page.props.rtl">
            ...
            <Sidebar position="right" v-model:visible="createForm" class="p-sidebar-md">
                <ColorForm :form-errors="errors" @close="createForm = false;" :title="'New Color'"/>
            </Sidebar>
            <Sidebar position="right" v-model:visible="editForm" class="p-sidebar-md">
                <ColorForm :form-errors="errors" :edit-flag="editForm" @close="editForm = false;" :color-id="currentId" :title="'Edit Color'"/>
            </Sidebar>
        </div>
    </app-layout>
</template>
...
<script>
    ...
    import ColorForm from '@/Pages/Admin/Forms/ColorForm'
        ...
        data() {
            return {
                ...
                options: {
                    enabled: true,
                    mode: 'pages',
                    perPage: this.carColors.meta.pagination.per_page,
                    setCurrentPage: this.carColors.meta.pagination.current_page,
        ...
        methods: {
            ...
            deleteSection(id) {
                this.$confirm.require({
                    ...
                    acceptLabel: 'Delete',
                    accept: () => {
                        this.$inertia.delete(route('colors.destroy', {id: id}), {}, {
                            onSuccess: () => {
                                this.$toast.add({
                                    severity: 'info',
                                    summary: 'Confirmed',
