    ...
    <div class="overflow-y-auto h-screen px-2">
        ...
        <form v-else class="my-6 w-11/12 mx-auto xl:w-full xl:mx-0" @submit.prevent="submitForm">
            ...
            <div class="w-full flex flex-col mb-6">
                <label for="name" class="pb-2 font-semibold text-gray-800">Brand Name <span class="ml-1 text-red-400">*</span></label>
                <InputText type="text"
                           id="name"
                           v-model="form.name"
            ...
            <!-- Logo Path -->
            <div class="w-full flex flex-col mb-6">
                <label for="logo_path" class="pb-2 font-semibold text-gray-800">Logo Path <span class="ml-1 text-red-400">*</span></label>
                <InputText type="text"
                           id="logo_path"
                           v-model="form.logo_path"
                           placeholder="Enter Logo Path" aria-describedby="name-help"
                           :class="[errors.logo_path ? 'p-invalid' : '']"

                />
                <small id="logo_path-help" v-if="errors.logo_path" class="p-invalid">{{ errors.logo_path }}</small>
            </div>
...
<script>
    ...
    export default defineComponent({
        ...
        methods: {
            ...
            update() {
                this.formValidated = true;
                this.$inertia.patch(route('brands.update', { id: this.brandId }), this.form, {
                    onSuccess: () => {
                        if (Object.keys(this.errors).length === 0) {
                            this.$emit('close', true);
                        }
            ...
            fetch() {
                if(this.editFlag) {
                    let _this = this;
                    _this.loading = true;
                    axios.get(route('brands.edit', { id: this.brandId }))
                        .then(function (response) {
                            let data = response.data;
                            _this.form.name = data.name;
                            _this.form.logo_path = data.logo_path;
