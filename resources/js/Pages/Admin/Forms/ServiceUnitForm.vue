    ...
    <div class="overflow-y-auto h-screen px-2">
        ...
        <form v-else class="my-6 w-11/12 mx-auto xl:w-full xl:mx-0" @submit.prevent="submitForm">
            ...
            <div class="w-full flex flex-col mb-6">
                <label for="name" class="pb-2 font-semibold text-gray-800">Name <span class="ml-1 text-red-400">*</span></label>
                <InputText type="text"
                           id="name"
                           v-model="form.name"
            ...
            <!-- Last Name -->
            <div class="w-full flex flex-col mb-6">
                <label for="number_plate" class="pb-2 font-semibold text-gray-800">Number Plate <span class="ml-1 text-red-400">*</span></label>
                <InputText type="text"
                           id="number_plate"
                           v-model="form.number_plate"
                           placeholder="Enter Number Plate" aria-describedby="number_plate-help"
                           :class="[errors.number_plate ? 'p-invalid' : '']"

                />
                <small id="last_name-help" v-if="errors.number_plate" class="p-invalid">{{ errors.number_plate }}</small>
            </div>
    ...
    export default defineComponent({
        ...
        methods: {
            ...
            create() {
                ...
                this.$inertia.post(route('service-units.store'), this.form, {
                    onSuccess: () => {
            ...
            fetch() {
                if(this.editFlag) {
                    let _this = this;
                    _this.loading = true;
                    axios.get(route('service-units.edit', { id: this.serviceUnitId }))
                        .then(function (response) {
                            let data = response.data;
                            _this.form.name = data.first_name;
                            _this.form.number_plate = data.last_name;
