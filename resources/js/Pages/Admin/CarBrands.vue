<template>
    <app-layout title="Car Brands">
        <template #header>
            <h4 class="page-heading">Car Brands</h4>
        </template>
        <template #actions>
        ...
        <div class="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <vue-good-table mode="remote" v-on:page-change="onPageChange" v-on:column-filter="onColumnFilter" v-on:per-page-change="onPerPageChange"
                            ...
                            :rows="carBrands.data" :rtl="$page.props.rtl">
                <template #table-row="props">

                    <!-- Name Column -->
                    <div class="flex items-center" v-if="props.column.field === 'name'">
                        <img class="w-12 h-8 mr-1" :src="props.row.logo_path" :alt="props.row.name">
        ...
        methods: {
            ...
            deleteSection(id) {
                this.$confirm.require({
                    ...
                    acceptLabel: 'Delete',
                    accept: () => {
                        this.$inertia.delete(route('brands.destroy', {id: id}), {}, {
                            onSuccess: () => {
                                this.$toast.add({
                                    severity: 'info',
                                    summary: 'Confirmed',
